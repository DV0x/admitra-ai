...
export function ActionCard({ message, onExecute, onCancel, isExecuting = false }: ActionCardProps) {
  ...
  if (result) {
    ...
    return (
      <motion.div
        ...
        className="bg-white shadow-sm shadow-black/5 border border-border rounded-2xl p-4"
      >
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className={`mt-3 flex items-center gap-2 text-sm ${result.success ? 'text-success' : 'text-error'}`}>
  ...
  if (isExecuting || instance.status === 'executing') {
    return (
      <motion.div
        initial={{ opacity: 0, y: 10 }}
        animate={{ opacity: 1, y: 0 }}
        className="bg-white shadow-md shadow-black/5 border-2 border-accent/40 rounded-2xl p-5 animate-pulse"
      >
        <div className="flex items-start gap-3">
  ...
  return (
    <motion.div
      initial={{ opacity: 0, y: 10 }}
      animate={{ opacity: 1, y: 0 }}
      className="bg-white shadow-md shadow-black/5 border border-border rounded-2xl p-5"
    >
