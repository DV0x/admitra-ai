...
interface WSServerMessage {
  ...
  message?: string;
  retryAttempt?: number;
  // P1: Cost tracking from complete event
  costUsd?: number;
  // P0: Notification hook fields
  title?: string;
}
...
interface WebSocketState {
  ...
  // Deferred action: hold action_instance until message completes (so text appears before ActionCard)
  deferredAction: ActionMessage | null;
}
...
export function useWebSocket() {
    ...
    (data: WSServerMessage) => {
      switch (data.type) {
        ...
        case 'block_delta':
          ...
          // Phase 7: Block system handles accumulation via block_delta events
          break;
        ...
        // P1: PostToolUse hook broadcasts tool completion events
        case 'tool_complete': {
          const toolName = data.toolName;
          if (toolName) {
            console.log('[WS] Tool complete:', toolName);
          }
          // Clear activity since tool finished
          setActivity(null);
          break;
        }

        // P0: Notification hook forwards agent status updates
        case 'notification': {
          const title = data.title || '';
          const message = data.message || '';
          if (title || message) {
            console.log('[WS] Notification:', title, message);
            setActivity(message || title);
          }
          break;
        }

        // AskUserQuestion: Agent is asking clarification questions
        case 'ask_user_question': {
          const questionId = (data as any).questionId as string;
          const questions = (data as any).questions as QuestionItem[];
          if (questionId && questions) {
            console.log('[WS] AskUserQuestion:', questionId, questions.length, 'question(s)');
            ...
            setState(prev => ({
              ...prev,
              messages: [...prev.messages, questionMsg],
            }));
          }
          break;
        }
        ...
        case 'action_instance': {
          ...
          // IMPORTANT: Don't add to messages yet! Defer until 'complete' so text appears first.
          ...
          // Store as deferred - will be added after text message on 'complete'
          setState((prev) => ({
            ...prev,
            deferredAction: actionMessage,
        ...
        case 'complete': {
          ...
          // P1: Extract costUsd from complete event
          const costUsd = data.costUsd;
          if (costUsd !== undefined) {
            console.log('[WS] Session cost: $' + costUsd.toFixed(4));
          }
          ...
          setState((prev) => {
            ...
            // Add deferred action AFTER text message (so text appears first, then ActionCard)
            if (prev.deferredAction) {
              updatedMessages = [...updatedMessages, prev.deferredAction];
            }
            ...
            return {
              ...
              executingActionId: null,
              // Clear deferred action after adding it
              deferredAction: null,
            };
          });
  ...
  const answerQuestion = useCallback(
    (questionId: string, answers: Record<string, string>) => {
      ...
      console.log('[WS] Answering question:', questionId, answers);
      ...
      // Mark question as answered in messages
      setState(prev => ({
        ...prev,
        activity: 'Processing your answer...',
        messages: prev.messages.map(msg =>
          msg.type === 'question' && (msg as QuestionMessage).questionId === questionId
            ? { ...msg, answers, isAnswered: true } as QuestionMessage
            : msg
        ),
      }));
    },
